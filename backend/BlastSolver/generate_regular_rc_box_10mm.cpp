...
int main(int argc, char** argv) {
    ...
    std::cout << "======================================================================" << std::endl;
    std::cout << "=== REGULAR RC HOLLOW BOX GENERATOR (MINIMUM 10MM ELEMENT SIZE) ======" << std::endl;
    std::cout << "======================================================================" << std::endl;
    ...
    const double min_elem_target = 0.010; // 10mm strictly enforced minimum
    const std::string out_filename = "regular_rc_box_10mm.k";

    std::cout << "Outer Dimensions: " << Lx << "m x " << Ly << "m x " << Lz << "m" << std::endl;
    std::cout << "Wall Thickness: " << t_wall * 1000.0 << " mm" << std::endl;
    std::cout << "Minimum Element Size: " << min_elem_target * 1000.0 << " mm (strictly enforced min(h) >= 10mm)" << std::endl;
    std::cout << "Rebar Spacing: " << rebar_pitch * 1000.0 << " mm (125mm c/c dual curtains + ties)" << std::endl;

    // 1. Build Partition Stations with Strict Piecewise Subdivisions ensuring min(h) >= 10.0mm
    auto build_stations = [&](double L, double tw, double cov, double pitch) {
        std::vector<double> st;

        // --- Wall 1: [0.0 ... 0.150m] ---
        // 1. Outer Cover: [0.0 ... 0.025m] (2 elements: 12.5 mm each)
        st.push_back(0.0);
        st.push_back(cov / 2.0); // 12.5mm
        st.push_back(cov);       // 25.0mm

        // 2. Wall 1 Core: [0.025 ... 0.125m] (10 elements: 10.0 mm each)
        for (int i = 1; i <= 10; ++i) {
            st.push_back(cov + i * ((tw - 2.0 * cov) / 10.0));
        }

        // 3. Wall 1 Inner Cover: [0.125 ... 0.150m] (2 elements: 12.5 mm each)
        st.push_back(tw - cov / 2.0); // 137.5mm
        st.push_back(tw);             // 150.0mm

        // --- Void: [0.150 ... 1.850m] ---
        // 4. Transition from wall boundary (0.150m) to first interior rebar station (0.250m)
        // Span = 100.0 mm (10 elements: 10.0 mm each)
        for (int i = 1; i <= 10; ++i) {
            st.push_back(tw + i * (0.100 / 10.0));
        }

        // 5. Interior 125mm Rebar Bays from 0.250m to 1.750m (12 bays)
        // Each 125mm bay divided into 12 elements: 10.4167 mm each
        ...
        for (int b = 0; b < num_interior_bays; ++b) {
            ...
            for (int sub = 1; sub <= 12; ++sub) {
                st.push_back(bay_start + sub * (pitch / 12.0));
            }
        }
        ...
        // Span = 100.0 mm (10 elements: 10.0 mm each)
        for (int i = 1; i <= 10; ++i) {
            st.push_back(1.750 + i * (0.100 / 10.0));
        }

        // --- Wall 2: [1.850 ... 2.000m] ---
        // 7. Wall 2 Inner Cover: [1.850 ... 1.875m] (2 elements: 12.5 mm each)
        st.push_back(L - tw + cov / 2.0); // 1862.5mm
        st.push_back(L - (tw - cov));     // 1875.0mm

        // 8. Wall 2 Core: [1.875 ... 1.975m] (10 elements: 10.0 mm each)
        for (int i = 1; i <= 10; ++i) {
            st.push_back((L - tw + cov) + i * ((tw - 2.0 * cov) / 10.0));
        }

        // 9. Wall 2 Outer Cover: [1.975 ... 2.000m] (2 elements: 12.5 mm each)
        st.push_back(L - cov / 2.0); // 1987.5mm
        st.push_back(L);             // 2000.0mm
    ...
    out << "$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n";
    out << "$ LS-DYNA KEYWORD DECK: REGULAR STRUCTURED RC HOLLOW BOX (2M, MIN 10MM ELEMENTS)\n";
    ...
    out << "*KEYWORD\n";
    out << "*TITLE\n";
    out << "Regular Structured RC Hollow Box 2m (Min 10mm Hexes, 125mm Rebar)\n";
    ...
    // Export VTK files
    {
        std::ofstream vtk("regular_concrete_10mm.vtk");
        vtk << "# vtk DataFile Version 3.0\nRegular Concrete 10mm (Min 10mm)\nASCII\nDATASET UNSTRUCTURED_GRID\n";
    ...
    {
        std::ofstream vtk("regular_rebar_10mm.vtk");
        vtk << "# vtk DataFile Version 3.0\nRegular Rebar 10mm\nASCII\nDATASET UNSTRUCTURED_GRID\n";
        vtk << "POINTS " << nodes.size() << " float\n";
        for (const auto& p : nodes) vtk << float(p.x) << " " << float(p.y) << " " << float(p.z) << "\n";
