...
void CFDSolver::updatePrimitiveFromConservative(std::vector<ConservativeState>& U_vec, std::vector<State>& states_vec) {
    ...
    for (int i = 0; i < limit; ++i) {
        ...
        if (!bad) {
            ...
            p = MultiMat::getMixturePressure(e_internal, U_vec[i].rho, alpha1, alpha2, arho1, arho2, gamma, currentMaterials.products, currentMaterials.unreacted);
        ...
        if (bad) {
            ...
            auto is_good = [&](int idx) {
                ...
                double p_n = MultiMat::getMixturePressure(std::max(e_internal_n, p_floor / (gamma - 1.0)), U_vec[idx].rho, U_vec[idx].alpha1, U_vec[idx].alpha2, U_vec[idx].arho1, U_vec[idx].arho2, gamma, currentMaterials.products, currentMaterials.unreacted);
            ...
            if (count > 0) {
                ...
                // The averaged arho1+arho2 can exceed rho_new if the
                // neighbour cells have a different mixture composition.
                // Enforce  arho1 + arho2 ≤ rho  before EOS calls.
                // -------------------------------------------------------
            ...
            states_vec[i].p   = std::max(MultiMat::getMixturePressure(
                e_int_n, states_vec[i].rho,
                states_vec[i].alpha1, states_vec[i].alpha2,
                states_vec[i].arho1,  states_vec[i].arho2,
...
ConservativeState CFDSolver::computedUdt(const std::vector<ConservativeState>& U_current, const std::vector<State>& states_current, int i, double dt) {
    double V_i = geom_V[i];
    double A_left = geom_A[i];
    double A_right = geom_A[i+1];

    ConservativeState F_left, F_right;
    ...
    if (i == 0 && bcLeft == REFLECTIVE) {
        ...
        F_left.E = 0.0; F_left.alpha1 = 0.0; F_left.alpha2 = 0.0; F_left.arho1 = 0.0; F_left.arho2 = 0.0;
        v_int[i] = 0.0;
    } else {
        double v_face_L = 0.0;
    ...
    if (i == n_cells - 1) {
        if (bcRight == TRANSMISSIVE) {
            ...
            F_right.E = 0; F_right.alpha1 = 0; F_right.alpha2 = 0; F_right.arho1 = 0; F_right.arho2 = 0;
            v_int[i+1] = 0.0;
        }
    } else {
        State sL, sR;
    ...
    // Well-balanced formulation: rearrange momentum update to avoid
    // catastrophic cancellation between the flux divergence pressure
    // contribution and the geometric source term p*(A_R - A_L)/V.
...
void CFDSolver::step(double dt) {
    ...
    if (temporalOrder == 1) {
        ...
        std::vector<ConservativeState> U1 = U, U2 = U;
        std::vector<State> states1 = states, states2 = states;
        ...
        // SSP-RK4 (Spiteri-Ruuth 5-stage SSP-RK4)
        std::vector<ConservativeState> U1 = U, U2 = U, U3 = U, U4 = U, U5 = U;
        std::vector<State> states1 = states, states2 = states, states3 = states, states4 = states, states5 = states;
    ...
    // Strang Splitting: Second half-step source
    applySourceTerms(0.5 * dt, currentTime + 0.5 * dt);


    // Dynamic boundary tracking: expand active region if perturbation reaches the buffer
    bool expand = false;
...
double CFDSolver::computeStepSize(double cfl) const {
    double max_u_c = 1e-6;
    double max_p_ratio = 1.0;
    int limit = std::min(n_cells, active_r_idx + 2);

    for (int i = 0; i < limit; ++i) {
        double c = MultiMat::getMixtureSoundSpeed(states[i].p, states[i].rho, states[i].alpha1, states[i].alpha2, states[i].arho1, states[i].arho2, gamma, currentMaterials.products, currentMaterials.unreacted);
...
void CFDSolver::run(double duration) {
    double elapsed = 0;
...
std::vector<double> CFDSolver::getLocalTimesteps(double cfl) const {
