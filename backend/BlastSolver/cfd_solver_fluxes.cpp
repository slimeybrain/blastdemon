...
ConservativeState CFDSolver::flux(const State& s) {
    ConservativeState f;
    f.rho = s.rho * s.u;
...
ConservativeState CFDSolver::getFlux(const State& sL, const State& sR, const ConservativeState& uL, const ConservativeState& uR, double dt, double& v_face) {
    ConservativeState f;
...
ConservativeState CFDSolver::getFluxRusanov(const State& sL, const State& sR, const ConservativeState& uL, const ConservativeState& uR, double& v_face) {
    ConservativeState fL = flux(sL);
    ConservativeState fR = flux(sR);

    double cL = MultiMat::getMixtureSoundSpeed(sL.p, sL.rho, sL.alpha1, sL.alpha2, sL.arho1, sL.arho2, gamma, currentMaterials.products, currentMaterials.unreacted);
    double cR = MultiMat::getMixtureSoundSpeed(sR.p, sR.rho, sR.alpha1, sR.alpha2, sR.arho1, sR.arho2, gamma, currentMaterials.products, currentMaterials.unreacted);
    ...
    if (u_int >= 0) {
        ...
        // Upwind partial density based on exactly computed global density flux to maintain unity
...
ConservativeState CFDSolver::getFluxAUSMPlus(const State& sL, const State& sR, double& v_face) {
    double aL = MultiMat::getMixtureSoundSpeed(sL.p, sL.rho, sL.alpha1, sL.alpha2, sL.arho1, sL.arho2, gamma, currentMaterials.products, currentMaterials.unreacted);
    double aR = MultiMat::getMixtureSoundSpeed(sR.p, sR.rho, sR.alpha1, sR.alpha2, sR.arho1, sR.arho2, gamma, currentMaterials.products, currentMaterials.unreacted);
...
void CFDSolver::reconstruct(const std::vector<State>& states_current, int i, State& s_L, State& s_R, double dt) {
    ...
    auto get_state = [&](int idx) {
        if (idx < 0) {
            State s = states_current[-idx]; // Fix ghost cell reflecting index
            s.u = -s.u;
            return s;
        }
    ...
    auto diff = [](const State& a, const State& b) {
    ...
    auto project_to_char = [](const State& ref, double c, const std::vector<double>& dV) {
        std::vector<double> dW(7);
        dW[0] = dV[0] - dV[2] / (c * c);
    ...
    auto project_to_prim = [](const State& ref, double c, const std::vector<double>& dW) {
        std::vector<double> dV(7);
    ...
    if (spatialOrder == 1) {
        ...
        State sm1 = get_state(i - 2);
        State s0  = get_state(i - 1);
        State sp1 = get_state(i);
        State sp2 = get_state(i + 1);

        // Reconstruction in cell i-1 (for s_L)
        double c0 = MultiMat::getMixtureSoundSpeed(s0.p, s0.rho, s0.alpha1, s0.alpha2, s0.arho1, s0.arho2, gamma, currentMaterials.products, currentMaterials.unreacted);
        auto dV_m1 = diff(s0, sm1);
        auto dV_0  = diff(sp1, s0);
        auto dW_m1 = project_to_char(s0, c0, dV_m1);
        auto dW_0  = project_to_char(s0, c0, dV_0);
        std::vector<double> dW_L(7);
        for(int k=0; k<7; ++k) dW_L[k] = limiter(dW_m1[k], dW_0[k]);
        ...
        double cp1 = MultiMat::getMixtureSoundSpeed(sp1.p, sp1.rho, sp1.alpha1, sp1.alpha2, sp1.arho1, sp1.arho2, gamma, currentMaterials.products, currentMaterials.unreacted);
        auto dV_p1 = diff(sp2, sp1);
        auto dW_p1 = project_to_char(sp1, cp1, dV_p1);
        auto dW_0_R  = project_to_char(sp1, cp1, dV_0);
        std::vector<double> dW_R(7);
        for(int k=0; k<7; ++k) dW_R[k] = limiter(dW_0_R[k], dW_p1[k]);
        ...
        if (s_L.rho < 1e-10 || s_L.p < 1e-10 || s_R.rho < 1e-10 || s_R.p < 1e-10) {
            ...
            s_L.E = MultiMat::getMixtureEnergy(s_L.p, s_L.rho, s_L.alpha1, s_L.alpha2, s_L.arho1, s_L.arho2, gamma, currentMaterials.products, currentMaterials.unreacted) + 0.5 * s_L.rho * s_L.u * s_L.u;
            s_R.E = MultiMat::getMixtureEnergy(s_R.p, s_R.rho, s_R.alpha1, s_R.alpha2, s_R.arho1, s_R.arho2, gamma, currentMaterials.products, currentMaterials.unreacted) + 0.5 * s_R.rho * s_R.u * s_R.u;
        }
    }
}
