...
void MPMSolver3D::gridToParticle(float dt) {
    ...
    for (auto& p : m_particles) {
        ...
        p.v[2] = std::clamp(v_pic_z, -5000.0f, 5000.0f);
...
void MPMSolver3D::updateStressState(float dt) {
    for (auto& p : m_particles) {
        ...
        // 3D Trial Elastic Stress Update
        float sig_trial[3][3];
        for (int r = 0; r < 3; ++r) {
            for (int c = 0; c < 3; ++c) {
        ...
        // 3D Hydrostatic Pressure & Deviatoric Stress
        ...
        if (q_trial > 1.0e-5f && yield_surf > 0.0f) {
            // 3D Radial Return Plastic Correction
        ...
        // Stress Softening
        float soft_factor = std::max(0.0f, 1.0f - p.damage);
        for (int r = 0; r < 3; ++r) {
            for (int c = 0; c < 3; ++c) {
